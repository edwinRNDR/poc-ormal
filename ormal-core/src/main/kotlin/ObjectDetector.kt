...
class ObjectDescription(val label: String, val score: Double, val bounds: Rectangle)
...
class ObjectDetector constructor(
    ...
    fun detect(colorBuffer: ColorBuffer): List<ObjectDescription> {
        ...
        for (i in 0 until ids.at(0).size()) {
            ...
            if (label >= 0) {
                ...
                result.add(ObjectDescription(synset[label], score.toDouble(), bounds))
            }
        }
        executor.dispose()
        args.remove("data")
...
fun objectDetectorFromUrls(context:Context, paramsUrl:String, symbolsUrl:String, synsetUrl:String) : ObjectDetector {
